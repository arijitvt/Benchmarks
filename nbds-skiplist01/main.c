...
void *thread_1(void *unused) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        sl_cas(sl_, key, CAS_EXPECT_WHATEVER, newVal);
    }
    return NULL;
}
...
void *thread_2(void *unused) {
    ...
    for (int i = 0; i < n; ++i) {
        map_key_t key = (i & (num_keys_ - 1)) + 1;
        sl_cas(sl_, key, CAS_EXPECT_WHATEVER, key);
    }
    return NULL;
}
...
int main (int argc, char **argv) {
    ...
    printf("Allocating skiplist\n");
    sl_ = sl_alloc(NULL);
    run_test();
    printf("Freeing skiplist\n");
    sl_free(sl_);
    printf("exiting\n");
    return 0;
...
void sl_print (skiplist_t *sl, int unused) {
...
#define   GET_NODE(x) ((node_ll_t *)(x))
#define STRIP_MARK(x) ((node_ll_t *)STRIP_TAG((x), 0x1))

static node_ll_t *node_ll_alloc (map_key_t key, map_val_t val) {
   node_ll_t *item = (node_ll_t *)nbd_malloc(sizeof(node_ll_t));
...
list_t *ll_alloc (const datatype_t *key_type) {
    ...
    ll->head = node_ll_alloc(0, 0);
    ll->head->next = DOES_NOT_EXIST;
    return ll;
}

void ll_free (list_t *ll) {
    node_ll_t *item = STRIP_MARK(ll->head->next);
    while (item != NULL) {
        node_ll_t *next = STRIP_MARK(item->next);
        if (ll->key_type != NULL) {
            nbd_free((void *)item->key);
        }
        nbd_free(item);
...
#ifdef LIST_USE_HAZARD_POINTER
static void nbd_free_node (node_ll_t *x) {
    nbd_free((void *)x->key);
    nbd_free(x);
}
#endif

static int find_pred (node_ll_t **pred_ptr, node_ll_t **item_ptr, list_t *ll, map_key_t key, int help_remove) {
    node_ll_t *pred = ll->head;
    node_ll_t *item = GET_NODE(pred->next);
...
map_val_t ll_cas (list_t *ll, map_key_t key, map_val_t expectation, map_val_t new_val) {
    ...
    do {
        ...
        if (!found) {
            ...
            map_key_t new_key = key;
            node_ll_t *new_item = node_ll_alloc(new_key, new_val);
...
void ll_print (list_t *ll, int verbose) {
    if (verbose) {
        ...
        while (next != DOES_NOT_EXIST) {
            node_ll_t *item = STRIP_MARK(next);
            if (item == NULL)
                break;
